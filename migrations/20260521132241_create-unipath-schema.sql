-- Generated for InsForge migration on 2026-05-21T18:53:26

-- SOURCE: C:\Users\Dr. Piyush Sir\Documents\unipath pj1\supabase\migrations\0001_unipath_supplyos.sql
...
for all using (public.has_role(array['admin', 'branch_admin', 'sales', 'material_team', 'dispatch_manager']::public.app_role[]) and public.can_access_branch(branch_id))
with check (public.has_role(array['admin', 'branch_admin', 'sales']::public.app_role[]) and public.can_access_branch(branch_id));
...
for all using (public.has_role(array['admin', 'branch_admin', 'dispatch_manager', 'material_team']::public.app_role[]) and public.can_access_branch(branch_id))
with check (public.has_role(array['admin', 'branch_admin', 'dispatch_manager', 'material_team']::public.app_role[]) and public.can_access_branch(branch_id));
...
for all using (
  exists (
    ...
    where d.id = dispatch_id
      ...
      and public.has_role(array['admin', 'branch_admin', 'dispatch_manager', 'material_team']::public.app_role[])
  )
)
with check (
  exists (
...
for all using (
  exists (
    ...
    where d.id = dispatch_id
      and public.can_access_branch(d.branch_id)
      and public.has_role(array['admin', 'branch_admin', 'dispatch_manager']::public.app_role[])
  )
)
with check (
  exists (
...
-- SOURCE: C:\Users\Dr. Piyush Sir\Documents\unipath pj1\supabase\migrations\0002_expiry_tracking.sql
...
-- SOURCE: C:\Users\Dr. Piyush Sir\Documents\unipath pj1\supabase\migrations\0003_role_model_alignment.sql
...
-- SOURCE: C:\Users\Dr. Piyush Sir\Documents\unipath pj1\supabase\migrations\0004_live_app_alignment.sql
